...
int main() {
  ...
  {
    const Eigen::SparseMatrix<double> sparse = matrix_laplacian.transpose()*matrix_laplacian+penalty_coeff*penalty_coeff*matrix_penalty;// write something here
  ...
  while (!glfwWindowShouldClose(viewer.window)) {
    ...
    // "vtx_xyz_ref", "vtx_xyz_def", "matrix_laplacian", "penalty_coeff", "matrix_penalty", and "solver" are used here.

     Eigen::MatrixXd rhs0 = matrix_laplacian.transpose()*(matrix_laplacian*vtx_xyz_ref)+penalty_coeff*penalty_coeff*matrix_penalty*vtx_xyz_def;// write some code to compute rhs vector
